...
const UserCart = () => {
  ...
  // Filter out any items where tour data is missing/null
  const validItems = cart?.items?.filter(item => item?.tour != null) ?? [];
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 pb-32 lg:pb-12">
      ...
      <div className="lg:hidden sticky top-0 z-50 bg-white border-b border-slate-100 px-4 py-3 flex items-center">
        ...
        <h1 className="text-base font-medium text-slate-900 ml-3 flex-1">Cart ({validItems.length})</h1>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 lg:px-6 py-6 lg:py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">{validItems.length} items selected</p>
          </div>
          {hasItems && (
                ...
                <div key={item._id} className="bg-white rounded-3xl border border-slate-100 p-4 lg:p-5 hover:border-slate-200 transition-all group">
                  <div className="flex flex-col sm:flex-row gap-5">
                    ...
                    <div className="w-full sm:w-44 h-32 sm:h-auto rounded-2xl overflow-hidden shrink-0">
                      <img
                        src={item.tour?.images?.[0]?.url || '/placeholder.jpg'}
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between items-start mb-2">
                        <h3 className="font-medium text-slate-900 leading-tight pr-4">
                      ...
                      <div className="flex items-center justify-between gap-4 mt-auto">
                        <div className="flex items-center bg-slate-50 rounded-xl p-1 border border-slate-100">
                          <button
                            onClick={() => handleUpdateTravelers(item._id, item.travelers - 1)}
                            disabled={item.travelers <= 1 || updating[item._id]}
                            className="w-8 h-8 flex items-center justify-center hover:bg-white rounded-lg transition-all disabled:opacity-30"
                          >
                            <Minus size={14} />
                          </button>
            ...
            <div className="lg:col-span-4 hidden lg:block sticky top-24">
              <div className="bg-slate-900 rounded-[2rem] p-8 text-white">
                ...
                <button
                  ...
                  className="w-full bg-amber-400 text-slate-900 py-4 rounded-2xl font-bold text-sm hover:bg-white transition-all flex items-center justify-center gap-2"
                >
      ...
      {hasItems && (
        <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-100 p-5 px-6 pb-8 z-50 flex items-center justify-between rounded-t-[2.5rem] shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
          ...
          <button
            onClick={() => navigate('/user/checkout')}
            className="bg-slate-900 text-amber-400 px-8 py-3.5 rounded-2xl font-bold text-sm flex items-center gap-2"
          >
