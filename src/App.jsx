      ...
      <AuthProvider>
        <CartProvider>
          <Router>
           ...
           /*     <Route path="/user" element={ // Changed from /user to /account for more standard UX
                  <ProtectedRoute>
                    <UserLayout />
                  </ProtectedRoute>
                }>
...
function App() {
  return (
    // HelmetProvider makes react-helmet-async's SEO meta tags work anywhere in the tree
    <HelmetProvider>
      {/* AuthProvider gives every child access to the logged-in user / auth actions */}
    /*  <AuthProvider>
        {/* CartProvider gives every child access to cart state (add/remove/checkout) */}
       /* <CartProvider>
          <Router>
...
/*            <ScrollToTop />
            <div className="min-h-screen bg-gray-50 selection:bg-amber-500/30">
              <Routes>
                ...
                {/* --- PUBLIC ROUTES (SEO Optimized Paths) --- */}
           /*  /*   {/* Everything under PublicLayout shares the public header/footer */}
           /*     <Route path="/" element={<PublicLayout />}>
      /*            <Route index element={<Home />} />
                  <Route path="tours" element={<Tours />} />
                  <Route path="tours/:id" element={<TourDetails />} />
             ...
             /*     <Route path="privacy-policy" element={<PrivacyPolicy />} />
                ...
                {/* --- USER PROTECTED ROUTES --- */}
                {/* ProtectedRoute redirects to /login if there's no authenticated user */}
           /*     <Route path="/user" element={ // Changed from /user to /account for more standard UX
                  <ProtectedRoute>
                    <UserLayout />
                  </ProtectedRoute>
                ...
                {/* --- ADMIN PROTECTED ROUTES --- */}
                {/* AdminRoute redirects non-admins away from everything under /admin */}
           /*     <Route path="/admin" element={
                  <AdminRoute>
                    <AdminLayout />
                  </AdminRoute>
